...
import com.example.admin_web_app.clients.orders.*;
import com.example.admin_web_app.config.SecurityHelper;
...
class OrderController {
    ...
    List<OrderSummary> getAllOrders() {
        ...
        return orderServiceClient.getAllOrders(getHeaders());
    }

    @PutMapping("/api/orders/admin/{orderNumber}/status")
    @ResponseBody
    void updateOrderStatus(@PathVariable String orderNumber, @RequestBody String status) {
        log.info("Updating order status for admin: {} -> {}", orderNumber, status);
        orderServiceClient.updateOrderStatus(getHeaders(), orderNumber, status);
    }
